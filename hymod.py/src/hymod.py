...
class Hymod:
    ...
    @staticmethod
    def calibrate(forcings,obs,paramSpace, precipCol="precip", petCol="pet", saveResults=False):
        samples = dict()
        ...
        for k in keyList:
            ...
            if "step" in subKeys:
                step = p["step"]
                samples[k] = np.arange(minV,maxV,step)
            elif "count" in subKeys:
                count = p["count"]
                samples[k] = np.linspace(minV,maxV,count)
            elif "samples" in subKeys:
                rando = p["samples"]
                samples[k] = np.random.uniform(minV,maxV,size=rando)

        grid = list(itertools.product(*[samples[k] for k in keyList]))
