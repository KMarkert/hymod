...
using DataFrames, Dates, Distributions, Distributed, TOML
...
include("Calibration.jl")
    ...
    function _power(x::Float64,y::Float64)::Float64
        x=abs(x) # Needed to capture invalid overflow with netgative values
        return x^y
    end

    function _excess(x_loss::Float64,cmax::Float64,bexp::Float64,prc::Float64,pet::Float64)
        xn_prev = x_loss
    ...
    function linearReservoir(x_slow::Float64,inflow::Float64,Rs::Float64)
        ...
        x_slow::Float64 = (1 - Rs) * x_slow + (1 - Rs) * inflow
        outflow::Float64 = (Rs / (1 - Rs)) * x_slow
        return x_slow,outflow
    end

    function hargreaves(forcings::DataFrame; tminCol=:tmin,tmaxCol=:tmax,dtCol=:datetime)

        dts = forcings[!,dtCol]
        tmin = forcings[!,tminCol]
        tmax = forcings[!,tmaxCol]
        len::UInt16 = length(tmax)
        Gsc::Float32 = 367
        lhov::Float32 = 2.257
    ...
    function simulate(forcings::DataFrame; precipCol=:precip,petCol=:pet,initFlow=true,cmax=1.0,bexp=0.0,alpha=0.2,ks=0.01,kq=0.5,saveResults=false)
        p = forcings[!,precipCol]
        e = forcings[!,petCol]

        lt_to_m::Float32 = 0.001
        ...
        x_slow::Float64 = initFlow ? (2.3503 / (ks * 22.5)) : 0
